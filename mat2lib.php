...
foreach ($data as $entry) {
    ...
    //if (str_contains($entry['thickness'], 'mm')) {
        ...
        $xmlString .= '<Entry Thickness="-1.0000" Desc="Spd: '.$entry['speedRoughRate'].' (Rough) Pce: '.$entry['pierceTime'].'" NoThickTitle="'.$entry['thickness'].'">';
        ...
        $xmlString .= '<speed Value="' . ((str_replace('in/min','',(str_replace('mm/min','',$entry['speedRoughRate']))))) / 60 . '"/>';
        ...
        $xmlString .= '<Material name="'.$entry['name'].'">';
        $xmlString .= '<Entry Thickness="-1.0000" Desc="Spd: '.$entry['speedMediumRate'].' (Medium) Pce: '.$entry['pierceTime'].'" NoThickTitle="'.$entry['thickness'].'">';
        $xmlString .= '<CutSetting type="Cut">';
        $xmlString .= '<index Value="0"/>';
        $xmlString .= '<name Value=""/>';
        $xmlString .= '<LinkPath Value="'.$entry['name'].'/'.$entry['thickness'].'/'.str_replace('&','and',$entry['category']).'"/>';
        ...
        $xmlString .= '<maxPower Value="100"/>';
        $xmlString .= '<maxPower2 Value="20"/>';
        $xmlString .= '<speed Value="' . ((str_replace('in/min','',(str_replace('mm/min','',$entry['speedMediumRate']))))) / 60 . '"/>';
        $xmlString .= '<startDelay Value="' . str_replace('s','',$entry['pierceTime']) . '"/>';
        $xmlString .= '<leadIn Value="1"/>';
        $xmlString .= '<leadLength Value="5"/>';
        $xmlString .= '<leadAngle Value="90"/>';
        ...
        $xmlString .= '<Material name="'.$entry['name'].'">';
        $xmlString .= '<Entry Thickness="-1.0000" Desc="Spd: '.$entry['speedFineRate'].' (Fine) Pce: '.$entry['pierceTime'].'" NoThickTitle="'.$entry['thickness'].'">';
        $xmlString .= '<CutSetting type="Cut">';
        $xmlString .= '<index Value="0"/>';
        $xmlString .= '<name Value=""/>';
        $xmlString .= '<LinkPath Value="'.$entry['name'].'/'.$entry['thickness'].'/'.str_replace('&','and',$entry['category']).'"/>';
